...
import sBB_functions_rules as sBB
import psutil

def spatialBB(PLFs_breakpoints_x, PLFs_breakpoints_y,
              n, K, problem, rhs,timelimit, epsilon, rule):
    '''
    ...
    rule : string
        branching rule type: "breakpoint","longest edge","largest error"
    ...
    while( len(nodes_lb) != 0 
        ...
        #--------------------------------------------------------------------
        child_nodes, comp_times = sBB.branch_and_bound(m,n, plf_values, env_breakpoints_x, env_breakpoints_y,PLFs_breakpoints_x, PLFs_breakpoints_y, problem, rule)
        ...
        # Case distincition if one side is infeasible
        if (left_model.status !=2):
            # the left side is infeasible
            #updating global upper bound
            if ub_right < global_ub:
                global_ub = ub_right
                incumbent = right_model.x[0:n]

        if (right_model.status !=2):
            # the right side is infeasible
            # updating global upper bound
            if ub_left < global_ub:
                global_ub = ub_left
                incumbent = left_model.x[0:n]
        else:
            # both side are feasible
        ...
        if (left_model.status == 2 and right_model.status ==2):
            if left_model.objval < global_ub:
                left_pos = bisect.bisect(nodes_lb,left_model.objval)
                nodes_lb.insert(left_pos, left_model.objval)
                nodes_model.insert(left_pos, left_model)
        ...
        if (left_model.status !=2 or right_model.status !=2):
            node_count += 1
        else:
            node_count += 2
