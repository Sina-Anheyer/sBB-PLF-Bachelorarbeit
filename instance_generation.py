...
Contains the functions to generate test problems and run computations. It includes:
    ...
    - getNetworkPLFs (generates concave PLFs for the network flow problem)
...
#--------------------------------------------------------------
# Funtion to generate discontinuous PLFs for the network flow problem
#--------------------------------------------------------------

def getDiscontinuousNetworkPLFs(n,K):
    ...
    for i in range(0,n):
        ...
        # x-values of PLF
        breakpoints_x = [0] # lower bound
        breakpoints_x.append(random.uniform(5,50)) # upper bound
        
        # Generate x-values and insert them in sorted order
        for k in range(1,K):
            new = random.uniform(breakpoints_x[0],breakpoints_x[-1])
            while new in breakpoints_x:
                new = random.uniform(breakpoints_x[0],breakpoints_x[-1])
        ...
        # y-values of PLF with fixed-charge jump at 0
        jump = random.uniform(10,50) # Generate fixed-charge jump at 0
        breakpoints_y = [jump]
...
#Function 21 (my first test function)
X21 = np.array([-5,5])
def func21(x): return 1/2*(x**2)-np.cos(x**2)
